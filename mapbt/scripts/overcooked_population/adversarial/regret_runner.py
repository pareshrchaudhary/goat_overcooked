...
class OvercookedRunner(Runner):
    ...
    def vae_train_with_multi_policy(self, reset_map_ea2t_ea2z_fn = None, reset_map_ea2p_fn = None):
        ...
        for episode in range(0, episodes): 
            ...
            if reset_map_ea2t_ea2z_fn is not None:
                ...
                episode_data = {"adversary_z": adversary_z,
                                "adversary_z_old": adversary_z_old,
                                "adversary_z_log_prob": adversary_z_log_prob}
            ...
            map_ea2z_adversary = {}
            for e in range(self.n_rollout_threads):
                map_ea2z_adversary[(e, 0)] = adversary_z[e].detach().cpu().numpy()
                map_ea2z_adversary[(e, 1)] = adversary_z[e].detach().cpu().numpy()
            
            episode_reward_vae_vs_vae = torch.zeros(self.n_rollout_threads, device=self.device)
            for step in range(self.episode_length):
                vae_actions = self.vae_agent.step(step, map_ea2z_adversary)  
                ...
                episode_reward_vae_vs_vae += torch.tensor([r.mean() for r in rewards], device=self.device)
            ##### VAE VS VAE #####
            ...
            # vae_vs_vae vs minimax train step 
            train_info = self.z_gen.train_step(episode_reward_vae_vs_vae, 
                                               episode_reward, 
                                               adversary_z, 
                                               adversary_z_old, 
                                               adversary_z_log_prob,
                                               z_current,
                                               z_prior)
            ...
            print("Average episode rewards Coordinator: {}".format({k.split('-')[0]: train_infos[k] 
    ...
    def train_vae_sp_minimax_regret(self):
