...
def parse_args(args, parser):
    ...
    parser.add_argument("--vae_z_generator", type=str, default="adversarial", choices=["normal_gaussian", "adversarial", "human_biased", "human_biased_std"], help="the type of the z generator")
    ...
    # adversary netwerk parameters
    parser.add_argument('--adversary_type', type=str, default='min', help='Type of adversary')
    parser.add_argument("--hidden_dim", default=128, type=int, help="number of hidden nodes in the adversary model")
    parser.add_argument("--use_mean_scaling", default=False, action='store_true', help="use mean scaling")
    parser.add_argument("--use_std_scaling", default=False, action='store_true', help="use std scaling")
    parser.add_argument("--scale_mean", default=60.0, type=float, help="scaling parameter for mean")
    ...
    parser.add_argument("--kl_coeff", default=0.1, type=float, help="beta for kl constraint")
    parser.add_argument("--use_entropy_bonus", default=False, action='store_true', help="use entropy bonus in the training")
    parser.add_argument("--entropy_bonus_coeff", default=0.01, type=float, help="beta for entropy bonus")

    # advesary optimizer parameters
    parser.add_argument("--adversary_learning_rate", default=0.001, type=int, help="learning rate of the z generator")
    parser.add_argument("--adversary_weight_decay", default=0.0001, type=int, help="weight decay of the z generator")
    parser.add_argument("--use_grad_clip", default=False, action='store_true', help="use gradient clip in the training")
    parser.add_argument("--grad_clip_norm", default=1.0, type=float, help="norm for gradient clip")
    ...
    parser.add_argument("--use_advanced_vizualization", default=False, action='store_true', help="use advanced visualization")
...
def main(args):
    ...
    # run dir
    run_dir = Path(os.getcwd()) / "results" / all_args.env_name / all_args.layout_name / all_args.algorithm_name / all_args.experiment_name
    ...
    if all_args.use_wandb:
        run = wandb.init(config=all_args,
                            ...
                            name=f"{all_args.algorithm_name}_{all_args.experiment_name}_seed{all_args.seed}_{datetime.now().strftime('%d-%m_%H:%M')}",
    ...
    if all_args.share_policy:
        from mapbt.scripts.overcooked_population.adversarial.vae_sp_runner import OvercookedRunner as Runner
    else:
        raise NotImplementedError
