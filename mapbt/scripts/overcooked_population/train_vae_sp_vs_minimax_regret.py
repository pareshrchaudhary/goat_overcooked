...
def main(args):
    ...
    if all_args.use_wandb:
        ...
        wandb.run.log_code(".", include_fn=lambda path: path.endswith("regret_runner.py") or path.endswith("regret_z_generator.py"))
    else:
        if not run_dir.exists():
            curr_run = 'run1'
        else:
    ...
    if all_args.share_policy:
        from mapbt.scripts.overcooked_population.adversarial.regret_runner import OvercookedRunner as Runner
    else:
        raise NotImplementedError
    ...
    runner.train_vae_sp_minimax_regret()
